...
// Local modification notice recorded 2026-09-19: macqd700-soc contributors
// added/threaded the default-off KEEP_FCS option for SONIC receive frames.
// Source Location: https://github.com/kpwn/macqd700-soc
/*
...
(
    parameter DATA_W = 8,
    // LOCAL MODIFICATION (macqd700-soc).  Emit the four FCS bytes on the AXI
    // stream instead of consuming them in the delay line.  The DP83932C
    // programming model requires the FCS present in the receive buffer, and
    // regenerating it downstream destroys the only end-to-end integrity check
    // in the design -- corruption between here and the driver gets re-blessed
    // as a valid frame.  Defaults off: unmodified behaviour unless asked for.
    parameter logic KEEP_FCS = 1'b0,
...
typedef enum logic [1:0] {
    ...
    STATE_PIPE,
    STATE_PAYLOAD,
    STATE_FCS       // KEEP_FCS: drain the 4 held FCS bytes out of the delay line
} state_t;
...
// KEEP_FCS drain: how many held bytes remain, and the tlast-time verdict
// latched at the instant gmii_rx_dv dropped.  The verdict must be captured
// there, not recomputed: update_crc is held low during the drain so crc_valid
// stays put, and the stats have already been emitted.
logic [2:0] fcs_cnt_reg = '0, fcs_cnt_next;
logic fcs_tuser_reg = 1'b0, fcs_tuser_next;
...
always_comb begin
    ...
    fcs_cnt_next = fcs_cnt_reg;
    fcs_tuser_next = fcs_tuser_reg;
        ...
        case (state_reg)
            ...
            STATE_PAYLOAD: begin
                ...
                if (!gmii_rx_dv) begin
                    ...
                    reset_crc = 1'b1;
                    if (KEEP_FCS) begin
                        // Defer end-of-frame by four bytes so the FCS still
                        // sitting in d1..d4 is emitted.  The verdict computed
                        // just above travels with it; tlast is withdrawn here
                        // and asserted on the final drained byte.
                        fcs_tuser_next = m_axis_rx_tuser_next;
                        m_axis_rx_tuser_next = 1'b0;
                        m_axis_rx_tlast_next = 1'b0;
                        fcs_cnt_next = 3'd3;
            ...
            STATE_FCS: begin
                // Drain the held FCS bytes.  update_crc stays low (it is only
                // raised in STATE_PAYLOAD) so crc_valid is undisturbed.
                m_axis_rx_tdata_next = gmii_rxd_d4_reg;
                m_axis_rx_tvalid_next = 1'b1;
                if (fcs_cnt_reg == 0) begin
                    m_axis_rx_tlast_next = 1'b1;
                    m_axis_rx_tuser_next = fcs_tuser_reg;
                    state_next = STATE_IDLE;
                end else begin
                    fcs_cnt_next = fcs_cnt_reg - 3'd1;
                    state_next = STATE_FCS;
                end
            end
            default: begin
...
always_ff @(posedge clk) begin
    ...
    fcs_cnt_reg <= fcs_cnt_next;
    fcs_tuser_reg <= fcs_tuser_next;
